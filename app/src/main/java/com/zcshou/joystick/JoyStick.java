...
public class JoyStick extends View {
    ...
    // 在类的成员变量中定义一个容器来存储位置数据
    List<Double> recordedLngs = new ArrayList<>();
    List<Double> recordedLats = new ArrayList<>();
    ...
    private void initJoyStickView() {
        ...
        mTimer.setListener(new GoUtils.TimeCount.TimeCountListener() {
            ...
            public void onFinish() {
                ...
                // 将计算得到的经度和纬度数据存储到容器中
                recordedLngs.add(disLng);
                recordedLats.add(disLat);
        ...
        // 录像按键的点击处理
        isRecording = false;
        btnRecord = mJoystickLayout.findViewById(R.id.joystick_record);
        btnRecord.setOnClickListener(v -> {
            if (!isRecording) {
                btnRecord.setColorFilter(getResources().getColor(R.color.colorAccent, mContext.getTheme()));
                isRecording = true;
                // 修改：开始记录位置数据
                recordedLngs.clear(); // 清空之前的数据
                recordedLats.clear(); // 清空之前的数据
            } else {
                btnRecord.setColorFilter(Color.argb(0, 0, 0, 0)); // 完全透明
        ...
        // 播放按键的点击处理
        isPlaying = false;
        btnPlay = mJoystickLayout.findViewById(R.id.joystick_play);
        btnPlay.setOnClickListener(v -> {
            if (!isPlaying) {
                btnPlay.setColorFilter(getResources().getColor(R.color.colorAccent, mContext.getTheme()));
                isPlaying = true;

                // 创建一个新的线程来播放记录的位置数据
                new Thread(new Runnable() {
                    int index = 0; // 记录当前读取的位置数据索引
                    Thread playbackThread; // 用于跟踪播放线程

                    @Override
                    public void run() {
                        // 检查是否有记录的位置数据
                        if (recordedLngs.isEmpty() || recordedLats.isEmpty()) {
                            return; // 没有记录的位置数据，不执行任何操作
                        }

                        // 循环播放记录的位置数据，同时检查播放按钮是否继续被点击
                        while (isPlaying) {
                            double lng = recordedLngs.get(index);
                            double lat = recordedLats.get(index);

                            // 将记录的经度和纬度数据传递给监听器进行定位
                            mListener.onMoveInfo(mSpeed, lng, lat, 90.0F - mAngle);

                            // 延时一段时间后继续播放下一个位置数据
                            try {
                                Thread.sleep(1000); // 假设延时 1 秒
                            } catch (InterruptedException e) {
                                e.printStackTrace();
                            }

                            index++; // 更新索引，准备读取下一个位置数据

                            // 如果index等于容器大小，则重新开始播放
                            if (index >= recordedLngs.size() || index >= recordedLats.size()) {
                                index = 0;
                            }
                        }
                    }
                ...
                btnPlay.setColorFilter(Color.argb(0, 0, 0, 0)); // 完全透明
                isPlaying = false;

                // 停止播放线程
                if (playbackThread != null && playbackThread.isAlive()) {
                    isPlaying = false; // 设置标志以停止线程
                    try {
                        playbackThread.join(); // 等待线程结束
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                }
